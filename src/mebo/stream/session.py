...
from mebo.stream.rtp import RTPStream

from mebo.stream.sdp import SDP
...
class RTSPSession:
    ...
    def __init__(
        self, url, port=None, username=None, realm=None, password=None, user_agent=None
    ):
        self.url = url
    ...
    def digest_response(self, nonce, method):
        return challenge_response(
            nonce, self._username, self._realm, self._password, method, self.url
        )
    ...
    def _challenge(self, resp):
        ...
        """
        if not resp.nonce:
            raise RTSPSessionError("Unable to find nonce in challenge header")
        ...
        try:
            ...
            raise DigestChallengeError(f"Unable to complete challenge: {e}")
        finally:
            self._nc += 1

    def _default_headers(self):
        return {"User-Agent": self._user_agent, "CSeq": self._cseq}
    ...
    def start_streams(self):
        ...
        self.streams = [
            RTPStream(media_desc, host=self.host) for media_desc in self.sdp.media
        ]
        ...
        if self.streams:
            play_response = self.play(
                **{"Session": self._session_id, "Range": "npt=0.000-"}
            )
            ...
            for stream in self.streams:
                ...
                t = threading.Thread(
                    name=f"{stream.sdp.name}-thread",
                    target=stream.capture,
                    args=(name,),
                    kwargs=dict(packets=4000),
                )
                t.start()
    ...
    def play(self, **kwargs):
        req = self._request_factory(
            "PLAY", self.url, **{"Session": self._session_id, "Range": "npt=0.000-"}
        )
        return self._request(req)

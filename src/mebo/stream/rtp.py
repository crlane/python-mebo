...
from mebo.stream.decode import RTPPacket, RTPDecodeError
...
SERVER_PORTS = re.compile(
    r"server_port=(?P<server_rtp>\d{4,5})-(?P<server_rtcp>\d{4,5})"
)
...
class RTPStream:
    ...
    def transport(self):
        ...
        return (
            f"{self.sdp.profile};unicast;client_port={self.media_port}-{self.rtcp_port}"
        )
